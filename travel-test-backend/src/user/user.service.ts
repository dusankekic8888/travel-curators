...
export class UserService {
  ...
  getUserIdFromToken(tokenString: string): number {
    try {
      const tokenDecode = this.authService.decodeToken(tokenString) as TokenDecode;
  ...
  private prepareUserUpdateObject(userUpdateDto: UserUpdateDto): UserUpdateDto {
    const { username, email, password, passwordOld, role, image } =
      userUpdateDto;

    return { username, email, password, passwordOld, role, image };
  }

  private buildUserResponseWithToken(user: UserEntity): ResUserWithTokenDto {
    const { id, username, email, image, role} = user;
    const token = this.authService.generateJWT(id.toString(), email, role);
    return {
      user: {
        username,
        email,
  ...
  private buildUserResponse(user: UserEntity): ResUserDto {
    const { username, email, image, role } = user;
    return {
      user: {
        username,
        role,
